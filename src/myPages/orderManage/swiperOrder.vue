<template>
  <div class="app-container swiperOrder-container">
    ...
    <el-table
      ...
      >
        <template slot-scope="scope">
          <a
            :href="scope.row.homeTopImgUrl"
            target="_blank"
            style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"
          >
            {{ scope.row.homeTopImgUrl }}
          </a>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <router-link class="link" :to="{path:'/accountManage/business',query:{id: scope.row.id}}">
            <p style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;margin:0">{{ scope.row.profit }}</p>
          </router-link>
        </template>
      </el-table-column>
    ...
    />
    <el-dialog title="轮播订单详情" :visible.sync="dialogFormVisible">
      <swiperOrderDetail :data-obj="detailData" />
    </el-dialog>
  </div>
</template>
  ...
  data() {
    return {
      ...
      form: {
        homeAdId: '',
        releaseTime: '',
        rangeType: 0, //  4: 默认首页推荐 5:全国好运 6:同城好运 7:附近好运
