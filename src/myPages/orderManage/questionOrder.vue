<template>
  <div class="app-container questionOrder-container">
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item label="发布范围" class="issueRange">
        <el-select v-model="form.rangeType" placeholder="请选择">
          <el-option
            v-for="item in rangTypes"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
    ...
    <el-table
      ...
      :data="tableData"
      :select-on-indeterminate="tabSelect"
      border
      fit
      ...
      <el-table-column align="center" prop="issueTime" label="发布时间" width="160">
        <template slot-scope="scope">
          <span v-if="scope.row.issueTime.length>0">{{ scope.row.issueTime[0] }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="状态" width="80">
        <template slot-scope="scope">
          <span>{{ scope.row.releasesSource == 1 ? '官方' : '用户' }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="问答订单详情" :visible.sync="dialogFormVisible">
      <questionOrderDetail :data-obj="detailData" />
    </el-dialog>
  </div>
</template>
